...
const securityPoints = [
  ...
  ["AI extraction sends selected chunks only.", "The pipeline does not send the full raw PDF file to OpenAI; references are excluded from extraction selection."],
  ["Evidence quotes allow verification.", "Every record can point back to a source quote, page, section, and document."],
  ["User OpenAI keys are encrypted.", "User-provided API keys are encrypted server-side and never returned in full."],
  ...
  return (
    <section className="marketing-container py-16 md:py-24">
      <SectionHeader
        ...
        description="ChemVault Extract avoids overstated compliance claims. The current system focuses on privacy defaults, project isolation, encrypted user API keys, evidence verification, and Stripe-hosted billing."
        align="center"
      />
