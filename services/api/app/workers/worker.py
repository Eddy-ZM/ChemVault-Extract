...
def process_job(job_id: str) -> None:
    ...
    with SessionLocal() as db:
        ...
        try:
            _record_status(db, job, JobStatus.QUEUED, "Worker picked up queued job")
            for next_status in (JobStatus.PARSING, JobStatus.CHUNKING, JobStatus.REVIEW_READY):
                time.sleep(settings.worker_step_delay_seconds)
                _record_status(db, job, next_status)
